...
int main() {
    ...
    big_integer zero(0);
    auto spread = vector<vector<big_integer>>(51, vector<big_integer>(1001, zero));

    for (int i = 0; i <= 18; i += 2) spread.at(1).at(i) = 1;
    for (int i = 1; i <= n; i++) spread.at(i).at(0) = 1;

    for (int i = 2; i <= n; i++) {
        for (int j = 2; j <= m; j += 2) {
            spread.at(i).at(j) = spread.at(i).at(j - 2) + spread.at(i - 1).at(j);
            if (j >= 20) spread.at(i).at(j) -= spread.at(i - 1).at(j - 20);
        }
    }

    cout << spread.at(n).at(m) * spread.at(n).at(m);
